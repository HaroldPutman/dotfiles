...
# For example:
# mvn-bundle-jar.sh -g com.adobe.aem -a uber-jar -c apis -f uber-jar-6.2.0-apis.jar

USAGE(){
    println "Usage: `basename $0` -g GroupId -a ArtifactId -c Classifier -f file [-voh] args\n\n";
...
while getopts hv:o:g:a:c:f: OPT; do
    case "$OPT" in
        h)
            USAGE
            exit 0
...
mvn install:install-file -Dclassifier=$CLASSIFIER -DlocalRepositoryPath=$TEMP_REPO_LOC -DgroupId=$GROUP_ID -DartifactId=$ARTIFACT_ID -Dversion=$VERSION -Dfile=$INPUT_FILE -Dpackaging=jar -DgeneratePom=true -DcreateChecksum=true
